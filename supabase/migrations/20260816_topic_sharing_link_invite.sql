-- Link-based sharing (no email required)
-- Run after 20260816_topic_sharing.sql

alter table public.topic_shares
  alter column shared_with_email drop not null;

alter table public.topic_shares
  drop constraint if exists topic_shares_status_check;

alter table public.topic_shares
  add constraint topic_shares_status_check
  check (status in ('pending', 'accepted', 'revoked', 'open'));

create table if not exists public.topic_share_members (
  id uuid primary key default gen_random_uuid(),
  share_id uuid not null references public.topic_shares (id) on delete cascade,
...
create index if not exists topic_share_members_user_idx
  on public.topic_share_members (user_id);

alter table public.topic_share_members enable row level security;

-- Helpers avoid RLS recursion between topic_shares <-> topic_share_members
...
-- Anyone signed in can read open invite links (needed to accept by token)
...
-- Members can also read their share link rows
...
-- Extend debate access for link members
